...
//             console.log('=== Checkout API Request Debug ===');
...
//             // Call unified API endpoint for both trial and paid
...
//             // Check if it's a free trial activation or payment
//             if (data.status === true && data.message === 'Free trail activated.') {
//                 // Free trial activated - redirect to confirmation
//                 router.push('/confirmation');
//             } else if (data.charge?.data?.transaction?.url) {
//                 // Payment flow - redirect to payment gateway
...
export default function CheckoutPage() {
    ...
    const handleDateChange = (date: Date | null) => {
        setDateOfBirthDate(date);
        if (date) {
            // Use local date formatted as DD-MM-YYYY to match backend Carbon format
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Prepare payload for unified API
            const payload = new FormData();
            payload.append('package_id', selectedPlan?.id || '0');
            payload.append('amount', selectedPlan?.price || '0');
            payload.append('first_name', formData.firstName);
            ...
            payload.append('city', formData.city);
            payload.append('post_code', formData.postCode);
            // Ensure date is clean ASCII string
            const cleanDate = formData.dateOfBirth.trim();
            payload.append('date_of_birth', cleanDate);
            payload.append('gender', formData.gender);
            payload.append('grade', formData.secondarySchoolGrade);
            payload.append('endpoint', 'confirmation');
            ...
            console.log('API Endpoint:', `${process.env.NEXT_PUBLIC_API_BASE_URL}/cms/tap/pay`);
            console.log('Raw date from state:', formData.dateOfBirth);
            console.log('Clean date:', cleanDate);
            console.log('Date string chars:', cleanDate.split('').map(c => c.charCodeAt(0)).join(','));
            ...
            // Call unified API endpoint - handles both trial and paid
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/cms/tap/pay`, {
                method: 'POST',
                headers: {
            ...
            try {
                ...
                // Try to get the HTML response to see the actual error
                try {
                    const textResponse = await responseClone.text();
                    console.error('=== Server Error Response ===');
                    console.error(textResponse.substring(0, 2000));
                    console.error('============================');

                    // If it's a short error, show it
                    if (textResponse.length < 200 && !textResponse.includes('<!DOCTYPE html>')) {
                        alert(`Server Error: ${textResponse}`);
                    } else {
                        // If it's likely HTML, look for Title
                        const titleMatch = textResponse.match(/<title>(.*?)<\/title>/i);
                        if (titleMatch) {
                            console.error('Error Page Title:', titleMatch[1]);
                            alert(`Server Error: ${titleMatch[1]}`);
                        } else {
                            alert('حدث خطأ في الخادم (500). يرجى التحقق من وحدة التحكم (Console) للتفاصيل.');
                        }
                    }
                } catch (textError) {
            ...
            if (data.status === true && data.message === 'Free trail activated.') {
                // Free trial activated - go to confirmation
                router.push('/confirmation');
            } else if (data.charge?.data?.transaction?.url) {
