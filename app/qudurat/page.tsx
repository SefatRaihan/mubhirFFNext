...
export default function ArSatPage() {
  ...
  const faqData = [
    ...
    },
    {
      ...
      answer: 'نعم، تقدر تبدأ بتجربة مجانية لمدة ثلاثة أيام للتعرف على طريقة عمل مبهر وتجربة بعض الأسئلة والميزات قبل الاشتراك.'
    },
    {
    ...
    const loadPlans = async () => {
      try {
        ...
        if (json?.status === 'success' && Array.isArray(json.data)) {
          ...
          const plans = json.data.filter((p: any) => p.package_type === 'SAT 1').map((plan: any) => ({
            ...plan,
          ...
          <div className="relative text-center mt-16 md:mt-[128px]">
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => {
                    Cookies.set('trialAudience', 'sat1', { path: '/' });
                    router.push('/signup');
                  }}
          ...
          <main className="max-w-6xl mx-auto flex flex-col px-0 md:px-4 py-6">
            <div id="plansGrid" className="flex flex-wrap justify-center gap-6 mx-auto">
              {pricingPlans.map((plan) => (
                <div key={plan.id} className="bg-white shadow-md rounded-2xl p-6 w-full md:w-[calc(50%_-_12px)] lg:w-[calc(33.333%_-_16px)] min-w-[300px] flex flex-col">
